...
But you can use an existing one if a signature of methods and data representation
are similar to existing one algorithms. For example, you can just copy necessary
files for your tutorial from a `DA3C` algorithm package.

In a respecting folder `relaax/algorithms/da3c/bridge` you find:
 - `bridge.py`: contains the main descriptions of the transport protocol
 and its methods and classes to communicate between agent and parameter server.
 - `bridge.proto`: description of your data structures which you use in `bridge.py`
...
As you can see, this class serves for both `parameter_server` and `agent`

We have methods for incrementing global step, apply gradients, and for retrieving a neural network weights.
We also use there some additional procedure to parse and built transmitted messages into `numpy` ndarrays.
