...
class Training(TrainingBase):
    ...
    def episode(self, number):
        ...
        for step in range(self.steps):
            ...
            print('state %s, action %s, reward %f' % (state, self.actions(action), reward))
            state = self.state()
            action = self.agent.update(reward=reward, state=state)

    def state(self):
        state = np.random.randint(0, self.action_size)
    ...
    def reward(self, action):
        ...
        for i in range(self.history_len):
            reward -= abs(actions[i] - self.ring_buffer[i])
        return reward

    def actions(self, action):
        actions = [None] * self.history_len
        for i in range(self.history_len):
            actions[i] = action % self.action_size
