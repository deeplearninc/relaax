...
class DA3CBatch(object):
    ...
    @profiler.wrap
    def reset(self):
        self.episode = episode.Episode('reward', 'state', 'action', 'value', 'probs')
        if da3c_config.config.use_lstm:
    ...
    def get_intrinsic_reward(self, state):
        ...
        if state is not None:
            ...
            intrinsic_reward = self.session.op_get_intrinsic_reward(state=icm_input, probs=[self.last_probs])
