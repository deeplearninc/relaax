...
def load():
    ...
    help='address of the Web Sockets server (host:port); in config yaml it is --address option in wsproxy section')
    parser.add_argument('--rlx-address', type=str, default=None, 
    help='address of the RLX server (host:port); in config yaml it is --bind option in relaax-rlx-server section')

    # Parse command line arguments
    args = parser.parse_args()

    # If parameters are not specified in command line try read them from config yaml
    if args.config:
    ...
    # If parameters are not set in command line or config, assume defaults 
    ...
    if len(args.ws_address) != 2:
        print "Error! Please specify Web Sockets server address in host:port format"
        exit()
