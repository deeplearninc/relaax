...
class LSTM(subgraph.Subgraph):
    def build_graph(self, x, batch_size=1, size=256):
        self.ph_step_size = graph.Placeholder(np.float32, [batch_size])

        self.ph_state = graph.TfNode(tuple(graph.Placeholder(np.float32, [batch_size, size])
                for _ in range(2)))

        self.zero_state = tf.contrib.rnn.LSTMStateTuple(
            *[np.zeros([batch_size, size]) for _ in range(2)])

        state = tf.contrib.rnn.LSTMStateTuple(*[ph.node for ph in self.ph_state.node])

        lstm = tf.contrib.rnn.BasicLSTMCell(size, state_is_tuple=True)

        with tf.variable_scope('LSTM') as scope:
            outputs, self.state = tf.nn.dynamic_rnn(lstm, x.node,
                    initial_state=state, sequence_length=self.ph_step_size.node,
