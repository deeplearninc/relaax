...
class PGAgent(object):
    ...
    def update(self,reward,state,terminal):
        ...
        if (episode_t == config.batch_size) or terminal:
            self.end_episode()
...
### Helper methods
    ...
    def update_shared_parameters(self, partial_gradients):
        self.ps.run("apply_gradients", feed_dict = {"gradients": partial_gradients})
    ...
    def load_shared_parameters(self):
        weights = ps.run("weights")
        sess.run(self.nn.assign_weights, feed_dict = {self.nn.shared_wights: weights})

    ### run agent's policy and get action
    def action_from_policy(self):
