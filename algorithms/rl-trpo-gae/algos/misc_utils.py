...
import six  # need for 2.5+ and 3+ compatibility --> iteritems
import numpy as np
...
GENERAL_OPTIONS = [
    ...
    ("use_hdf", int, 0, "whether to make an hdf5 file with results and snapshots"),     # CHANGE
...
def update_default_config(tuples, usercfg):
    ...
    for (name, _, defval, _) in tuples:
        out[name] = defval
    if usercfg:
        for (k, v) in usercfg.iteritems():    # old
        # for (k, v) in six.iteritems(usercfg):
            if k in out:
                out[k] = v
    return out
...
def prepare_h5_file(args):
    ...
    if osp.exists(fname) and fname != outfile_default:
        raw_input("output file %s already exists. press enter to continue. (exit with ctrl-C)" % fname)     # FIX
    import h5py
