...
class TrpoUpdater(EzFlat, EzPickle):
    ...
    def __init__(self, stochpol, usercfg, session):
        ...
        adv_n = tf.placeholder(dtype, name='adv_n')     # shape=[None],
        ...
        N = tf.shape(ob_no)[0]    # TF can use reduce_* functions
        ...
        # kl_firstfixed = tf.reduce_sum(probtype.kl(prob_np_fixed, prob_np))    # no norm
        kl_firstfixed = tf.reduce_sum(probtype.kl(prob_np_fixed, prob_np)) / N_float
        ...
        flat_tangent = tf.placeholder(dtype, name='flat_tan')   # shape=[None],
        ...
        self.compute_policy_gradient = TensorFlowTheanoFunction(args, pg, session)
        ...
        self.compute_fisher_vector_product = TensorFlowTheanoFunction([flat_tangent] + args, fvp, session)
...
def cg(f_Ax, b, cg_iters=10, callback=None, verbose=False, residual_tol=1e-10):
    ...
    for i in xrange(cg_iters):  # FIX later for compatibility
